...
switch ($action) {
    ...
    // ========================================
    // API SAGE (Enterprise only)
    // ========================================
    case 'sage_sync':
        $me = requireAuth('comptable');
        $db = readJSON('fleet.json') ?: newDB();
        // Generate SAGE-compatible XML/JSON for accounting sync
        $sageData = [
            'period' => isset($_GET['month']) ? $_GET['month'] : date('Y-m'),
            'entries' => []
        ];
        // Export all fuel entries as accounting entries
        foreach ($db['fuel'] as $f) {
            if (substr($f['date'], 0, 7) === $sageData['period']) {
                $sageData['entries'][] = [
                    'date' => $f['date'],
                    'account' => '6132', // Carburant
                    'label' => 'Carburant ' . $f['truckRef'] . ' ' . ($f['ticketNo'] ?? ''),
                    'debit' => ($f['liters'] ?? 0) * ($f['pricePerL'] ?? 0),
                    'credit' => 0,
                    'journal' => 'ACH'
                ];
            }
        ...
        foreach ($db['oilChanges'] as $o) {
            if (substr($o['date'], 0, 7) === $sageData['period']) {
                $sageData['entries'][] = [
                    'date' => $o['date'],
                    'account' => '6155', // Entretien
                    'label' => 'Vidange ' . $o['truckRef'] . ' ' . ($o['ticketNo'] ?? ''),
                    'debit' => ($o['liters'] ?? 0) * ($o['pricePerL'] ?? 0),
                    'credit' => 0,
                    'journal' => 'ACH'
                ];
            }
        ...
        foreach ($db['filterChanges'] as $f) {
            if (substr($f['date'], 0, 7) === $sageData['period']) {
                $sageData['entries'][] = [
                    'date' => $f['date'],
                    'account' => '6155', // Entretien
                    'label' => 'Filtre ' . $f['truckRef'] . ' ' . ($f['ticketNo'] ?? ''),
                    'debit' => ($f['qty'] ?? 0) * ($f['price'] ?? 0),
                    'credit' => 0,
                    'journal' => 'ACH'
                ];
            }
        }
        jsonResponse(['sage_export' => $sageData, 'count' => count($sageData['entries'])]);
        break;
    ...
    case 'executive_dashboard':
        ...
        // KPIs temps reel
        $totalTrucks = count($db['trucks']);
        $activeTrucks = count(array_filter($db['trucks'], function($t){return ($t['statut'] ?? '') === 'Actif';}));
        $maintTrucks = count(array_filter($db['trucks'], function($t){return ($t['statut'] ?? '') === 'Maintenance';}));
        $panTrucks = count(array_filter($db['trucks'], function($t){return ($t['statut'] ?? '') === 'En panne';}));
        
        // Cout par mois (12 derniers)
        $monthlyCosts = [];
        for ($i = 0; $i < 12; $i++) {
            $month = date('Y-m', strtotime("-$i months"));
            $cost = 0;
            foreach ($db['fuel'] as $f) if (substr($f['date'], 0, 7) === $month) $cost += ($f['liters'] ?? 0) * ($f['pricePerL'] ?? 0);
            foreach ($db['oilChanges'] as $o) if (substr($o['date'], 0, 7) === $month) $cost += ($o['liters'] ?? 0) * ($o['pricePerL'] ?? 0);
            foreach ($db['filterChanges'] as $f) if (substr($f['date'], 0, 7) === $month) $cost += ($f['qty'] ?? 0) * ($f['price'] ?? 0);
            $monthlyCosts[] = ['month' => $month, 'cost' => round($cost, 3)];
        }
        ...
        foreach ($db['trucks'] as $t) {
            $tc = 0;
            foreach ($db['fuel'] as $f) if ($f['truckRef'] === $t['ref']) $tc += ($f['liters'] ?? 0) * ($f['pricePerL'] ?? 0);
            foreach ($db['oilChanges'] as $o) if ($o['truckRef'] === $t['ref']) $tc += ($o['liters'] ?? 0) * ($o['pricePerL'] ?? 0);
            foreach ($db['filterChanges'] as $f) if ($f['truckRef'] === $t['ref']) $tc += ($f['qty'] ?? 0) * ($f['price'] ?? 0);
            $truckCosts[] = ['truck' => $t['ref'], 'cost' => round($tc, 3)];
        }
        usort($truckCosts, function($a, $b){return $b['cost'] <=> $a['cost'];});
        
        jsonResponse([
            'kpis' => [
                'total_trucks' => $totalTrucks,
                'active' => $activeTrucks,
                'maintenance' => $maintTrucks,
                'breakdown' => $panTrucks,
                'availability_rate' => $totalTrucks > 0 ? round(($activeTrucks / $totalTrucks) * 100, 1) : 0
            ],
            'monthly_costs' => $monthlyCosts,
            'top_truck_costs' => array_slice($truckCosts, 0, 10),
            'currency' => $currency
        ]);
        break;
